...
class RPCServer extends Base implements Swoole\IFace\Protocol
{
    ...
    {
        SLog::info(__METHOD__ . '-------'.$worker_id);
        $this->log("Worker[$worker_id] is stop");
    }

    function onShutdown($server)
    {
        SLog::info(__METHOD__ . '-------');
    ...
    {
        SLog::info(__METHOD__ . '-------'.$interval);
        $this->log("Timer[$interval] call");
    }

    function onReceive($serv, $fd, $reactor_id, $data)
    {
        SLog::info(__METHOD__ . '-------'.$fd);
        SLog::info(__METHOD__ . '-------'.$reactor_id);
        ...
        else
        {
            ...
            SLog::info('Response:'.json_encode([$response,$ret]));
            if ($ret === false)
            {
    ...
    {
        SLog::info(__METHOD__ . '-------'.self::$clientEnv);
    ...
    {
        SLog::info(__METHOD__ . '-------'.self::$requestHeader);
        return self::$requestHeader;
    }

    function sendErrorMessage($fd, $errno)
    {
        SLog::info(__METHOD__ . '-------'.json_encode([$fd, $errno]));
    ...
    {
        SLog::info(__METHOD__ . '-------'.json_encode($data));
        //启用压缩
        ...
        switch($_type)
        {
            ...
            case self::DECODE_SWOOLE:
                //$body = \swoole_serialize::pack($data);  //原代码
                $body = serialize($data);
                break;
        ...
        SLog::info('打包数据数据源:'.$body);
        $endata = pack(RPCServer::HEADER_PACK, strlen($body), $type, $uid, $serid) . $body;
        SLog::info('打包数据二进制:'.$endata);
        return $endata;
    }
    ...
    static function decode($data, $unseralize_type = self::DECODE_PHP)
    {
        ...
        switch ($unseralize_type)
        {
            ...
            case self::DECODE_SWOOLE:
//                return \swoole_serialize::unpack($data); //原代码
    ...
    {
        SLog::info(__METHOD__ . '-------'.json_encode([$fd, $from_id]));
    ...
    {
        SLog::info(__METHOD__ . '-------'.json_encode([$name, $path]));
        if (!is_dir($path))
        {
    ...
    {
        SLog::info(__METHOD__ . '-------'.json_encode([$request, $header]));
        ...
        //调用接口方法
        SLog::info('接口入参:'.json_encode($request['params']));
        ...
        return array('errno' => 0, 'data' => $ret, 'params' => $request['params']);
    }
